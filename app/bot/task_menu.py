...
def main_reply_kb(connected: bool = True) -> ReplyKeyboardMarkup:
    ...
    # Кнопку запуска Mini App держим не здесь, а как menu button у поля ввода
    # (bot.set_chat_menu_button в main.py) — иначе нижнее меню разрастается.
    return ReplyKeyboardMarkup(
        keyboard=[
...
WHATS_NEW = (
    ...
    f"📊 <b>Лимит/день</b> — до <b>{FREE_DAILY_LIMIT}</b> бесплатно и до <b>{PAID_DAILY_LIMIT}</b> на расширенном тарифе 💎.\n"
...
async def cb_ab_toggle(cb: CallbackQuery, state: FSMContext, **kw):
    async with async_session() as session:
        ...
        if not user.is_paid and not _is_admin(user.telegram_id):
            await cb.answer("A/B-тест писем — в расширенном тарифе 💎", show_alert=True)
            return
...
async def cb_source(cb: CallbackQuery, state: FSMContext, **kw):
    async with async_session() as session:
        holder, user, s = await _res(session, cb, state)
        if not user.is_paid and not _is_admin(user.telegram_id):
            await cb.answer("Рекомендации hh — в расширенном тарифе 💎", show_alert=True)
            return
...
_PROMPTS = {
    ...
    "daily_limit": "Пришли лимит откликов в день числом.\n"
                   f"Максимум на задачу: <b>{FREE_DAILY_LIMIT}</b> (бесплатно) / "
                   f"<b>{PAID_DAILY_LIMIT}</b> (расширенный тариф 💎).\n\n"
                   f"⚠️ Это лимит на задачу. Суммарно по ВСЕМ задачам бот отправит "
                   f"не больше <b>{PAID_DAILY_LIMIT}</b> в день, чтобы hh не забанил "
                   f"аккаунт за перебор.",
    ...
    "contact": "Пришли свои контакты для писем — их увидит HR. Можно сразу несколько "
               "через запятую: <code>you@mail.com, tg @your_tg, +7 900 000-00-00</code>. "
               "Это заменит контакты целиком, так что впиши все, что нужно (почта, ТГ, телефон). "
               "Пусто/<code>-</code> — бот возьмёт контакты из твоего резюме сам.",
...
async def cb_tariff(cb: CallbackQuery, **kw):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"Оформить за {settings.subscription_price}₽",
