...
class Settings(BaseSettings):
    ...
    # Модель для писем (всем). Пусто → claude-haiku-4-5. Качество письма даёт
    # промпт, а не мощность модели, поэтому письма гоняем на дешёвой.
    ai_letter_model: str = ""
    # Отдельный эндпоинт для писем (попробовать другую модель, напр. gpt-4.1-mini).
    # Если заданы все три — письма идут СНАЧАЛА сюда, при сбое откат на пул/Haiku.
    # Пусто → письма как обычно (бесплатный пул + фолбэк).
    ai_letter_base_url: str = ""
    ai_letter_api_key: str = ""
