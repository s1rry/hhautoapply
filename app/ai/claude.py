...
{vacancy_description}"""
    ...
    def _humanize(text: str) -> str:
        ...
        """
        # Срезать вводную болтовню модели ("Понял задачу. Вот письмо:") —
        # некоторые модели пула добавляют её перед письмом, невзирая на промпт.
        text = re.sub(
            r"^\s*(понял[аи]?\b[^\n]*|конечно\b[^\n]*|вот\b[^\n]*письмо[^\n:]*:?|"
            r"хорошо\b[^\n]*:|отлично\b[^\n]*:)\s*\n+",
    ...
    async def _score_call(self, system: str, user_msg: str) -> str | None:
        ...
        """
        # Скоринг идёт ТОЛЬКО по бесплатному пулу: объёмы огромные (тысячи/сутки),
        # а оценка вакансии не видна работодателю — платить за неё незачем.
        # gpt-4.1-mini (primary) используется только для писем.
        import time as _t
        now = _t.time()
        errors = []
        for i in range(len(self.score_pool)):
