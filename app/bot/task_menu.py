...
_PROMPTS = {
    ...
    "contact": "Пришли контакты для писем — их увидит HR. Можно сразу несколько "
               "через запятую: <code>i.egorov8080@gmail.com, tg @egorov_analyst</code>. "
               "Это заменит контакты целиком, так что впиши все, что нужно (почта, ТГ, телефон). "
               "Пусто/<code>-</code> — бот возьмёт контакты из резюме сам.",
